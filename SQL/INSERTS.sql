...
SELECT DISTINCT
       ...
       av.IdAplicacao                            -- Chave gerada automaticamente em AplicacaoVacina (IDENTITY)
FROM vacinacao_jan_2025 v   -- Tabela de origem importada via BULK INSERT
INNER JOIN AplicacaoVacina av
       ON av.CodigoDocumento = CAST(v.st_documento AS UNIQUEIDENTIFIER)  -- Relacionamento pelo documento
       AND av.CodigoPaciente = CAST(v.co_paciente AS UNIQUEIDENTIFIER)    -- Relacionamento pelo paciente
