...
INNER JOIN vacina v on v.CodigoVacina = vj.co_vacina -- Join para garantir que sejam inseridas somente as vacinas da tabela vacina
INNER JOIN VacinaFabricante vf on vf.CodigoVacinaFabricante = vj.co_vacina_fabricante -- Join para garantir que sejam inseridos somente os fabricantes já inseridos
WHERE vj.co_vacina_fabricante IS NOT NULL  -- Filtro para evitar campos nulos
...
INNER JOIN Vacina V ON VJ.co_vacina = V.CodigoVacina -- Garante somente vacinas já inseridas na tabela VACINA
INNER JOIN DoseVacina DV ON VJ.co_vacina = DV.CodigoDoseVacina -- Garante somente Doses já inseridas na tabela DoseVacina
...
FROM vacinacao_jan_2025 VJ
INNER JOIN Vacina V ON V.CodigoVacina = VJ.co_vacina -- Garante somente vacinas já inseridas na tabela VACINA
INNER JOIN ViaAdministracao VA ON VA.CodigoViaAdministracao = VJ.co_via_administracao -- Garante somente registros já inseridos na tabela ViaAdministracao
